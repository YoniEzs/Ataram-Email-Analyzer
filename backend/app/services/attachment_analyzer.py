        ...
        try:
            with zipfile.ZipFile(io.BytesIO(data)) as zf:
                ...
                if len(members) > self.max_archive_members:
                    issues.append('archive_too_many_members')
                    escalate('critical')

                total_uncompressed = sum(max(0, info.file_size) for info in members)
                total_compressed = sum(max(0, info.compress_size) for info in members)
                ratio = total_uncompressed / max(1, total_compressed)
                if (
                    total_uncompressed > self.max_archive_uncompressed_bytes
                    or (total_uncompressed > 1024 * 1024 and ratio > self.max_archive_ratio)
                ):
                    issues.append('archive_bomb_suspected')
                    escalate('critical')

                for info in members[:self.max_archive_members]:
