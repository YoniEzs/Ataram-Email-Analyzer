...
Scores are intentionally modest for the header-only samples: they exercise
signals in isolation, and the analyzer's trust model refuses to score anything
an attacker could fake by editing the file. The one exception is deliberate —
a locally-verified critical attachment is non-forgeable evidence, so sample 05
alone reaches "medium". Real phishing usually stacks these signals with URL
and content indicators, which is when scores climb further.
